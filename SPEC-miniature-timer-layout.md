...
- Miniature layout always uses a plain one-line numeric timer, even if ASCII timer art is enabled for the full layout.
- Preserve the existing full-layout width budget: `fullBudget = max(0, min(terminalWidth - 2*padding - margin, maxWidth))` (currently `max(0, min(terminalWidth - 8, 80))`). In miniature mode, use `barWidth = max(0, fullBudget - lipgloss.Width(plainTimer) - 1)`, where the final cell is the single-space timer/bar gap.
- Derive the miniature progress-bar width from the current plain miniature timer on every miniature render or timer/layout update. This covers toggles, terminal resizes, accepted timer ticks, duration increase/reset, and work/break/long-break/short-session transitions. Restore `fullBudget` when leaving miniature mode.
...
- Changes to the configured ASCII-art behavior in the full layout.
...
5. **Width mutation paths:** miniature rendering recalculates the exact `max(0, fullBudget - plainTimerWidth - 1)` bar budget after an accepted tick, duration increase, reset, and each session transition, including transitions that cross the one-hour formatting boundary.
...
7. **ASCII-art override:** with ASCII art enabled, miniature layout still renders a one-line numeric timer beside the progress bar, and bases the bar width on that plain timer.
...
- **Ask first:** changing the miniature content beyond its plain numeric timer plus bar; adding a persistent configuration option; changing key bindings; adding dependencies; changing normal-layout behavior.
...
- Miniature layout always renders a plain one-line numeric timer, regardless of the ASCII-art configuration; the full layout retains its configured ASCII-art behavior.
- The miniature bar width is exactly `max(0, max(0, min(terminalWidth - 8, 80)) - lipgloss.Width(plainTimer) - 1)`; the full layout restores `max(0, min(terminalWidth - 8, 80))`.
...
- The libraries document width APIs, but do not define this product's timer-only narrow-window fallback or ultra-narrow clipping behavior; those policies are specified here by user decision.
